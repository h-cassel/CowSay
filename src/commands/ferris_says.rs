...
impl FerrisSays {
    ...
    pub async fn run<'a>(&self, _options: &[ResolvedOption<'a>]) -> String {
        let out = if let Some(ResolvedOption {
            value: ResolvedValue::String(output),
            ..
        }) = _options.first()
        {
            *output
    ...
    pub fn register() -> CreateCommand {
        CreateCommand::new("ferris-says")
            .description("A visit from Ferris! What does he have to say?")
            .add_option(CreateCommandOption::new(
                CommandOptionType::String,
                "dialogue",
                "What should Ferris say?",
            ))
    }
}
